...
/// in base adapter we attached listeners which is listening to the call backs
// when I get call backs we are passing that to the viewholder
// Iam  manually setting the lifecycle based on the visibility in the screen
/*Iam passing the parent lifecycle in the baseadapter to check if activity is in onstoped state and avoid
unnecessary network calls
* When activity is stopped baseadapter lifecycle observer will have that event and tell the viewholder that lifecycle
is in stopped state
* So based on the lifecycle of activity viewholder always becomes lifecycle aware which also helps us to use livedata
in Itemviewmodel which needs lifecycle*/
